  ...
  if (!guest)
    return (
      <div className="min-h-screen grid place-items-center p-8 text-center">
        <div>
          <p className="display text-2xl mb-2">No card yet</p>
          <p className="text-muted mb-4">Scan the QR on your table and join rewards from the "Your table" tab.</p>
          <Link to="/m/t1" className="text-flame font-semibold">Open the demo menu →</Link>
        </div>
      </div>
    );

  const c = card ?? guest;
  const progress = c.nextTier ? Math.min(1, c.totalSpent / (c.totalSpent + c.nextTier.remaining)) : 1;
  ...
  return (
    <div className="min-h-screen max-w-lg mx-auto px-5 pt-6 pb-16">
      ...
      <div className="mt-4 rounded-3xl bg-gradient-to-br from-flame to-[#B33A0E] p-6 text-ink relative overflow-hidden">
        <p className="text-[11px] tracking-[0.25em] font-bold opacity-70">
          QARTA REWARDS · {c.tier.label.toUpperCase()}
        </p>
        <p className="display text-4xl font-extrabold mt-3">${pts(c.points)}</p>
        <p className="text-sm font-semibold opacity-70">points on your card</p>
      ...
      <div className="mt-5 rounded-2xl border hairline p-5">
        <div className="flex items-center justify-between text-sm">
          <span className="flex items-center gap-1.5 font-semibold">
            <TrendingUp size={15} className="text-flame" /> {Math.round(c.tier.rate * 100)}% cashback
          </span>
          {c.nextTier && (
            <span className="text-muted text-xs">
              {c.nextTier.label} ({Math.round(c.nextTier.rate * 100)}%) in ${c.nextTier.remaining.toFixed(0)}
            </span>
          )}
        </div>
        ...
        <p className="text-xs text-muted mt-2">Lifetime spend: ${c.totalSpent.toFixed(2)}</p>
      </div>

      {/* history */}
      <h2 className="display text-lg font-semibold mt-7 mb-3">History</h2>
      <div className="space-y-2">
        {card?.transactions?.length ? (
          ...
          <p className="text-muted text-sm">No activity yet — cashback lands when your bill is paid.</p>
        )}
      </div>
    </div>
  );
