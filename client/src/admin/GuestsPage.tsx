...
export default function GuestsPage() {
  ...
  return (
    <div>
      <h1 className="display text-2xl font-extrabold mb-6">Гости · CRM</h1>

      <div className="overflow-x-auto border-y hairline">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-muted text-xs">
              <th className="py-3 pr-4 font-medium">Гость</th>
              <th className="py-3 pr-4 font-medium">Телефон</th>
              <th className="py-3 pr-4 font-medium">Визиты</th>
              <th className="py-3 pr-4 font-medium">Потрачено</th>
              <th className="py-3 pr-4 font-medium">Баллы</th>
              <th className="py-3 font-medium">Уровень</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[rgba(245,243,238,0.09)]">
            {guests?.map((g) => (
              <tr key={g.id}>
                <td className="py-3 pr-4 font-semibold">{g.name ?? 'Гость'}</td>
      ...
      {guests?.length === 0 && <p className="text-muted mt-4">Гостей пока нет.</p>}
    </div>
  );
}
